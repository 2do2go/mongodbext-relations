...
exports.setup = function(collection, relations) {
	collection.on('beforeInsertOne', function(params, callback) {
		params.obj = utils.deepClone(params.obj);
		processEmbeddedDocuments(params.obj, relations, callback);
	});

	collection.on('beforeInsertMany', function(params, callback) {
		params.objs = utils.deepClone(params.objs);
		processEmbeddedDocuments(params.objs, relations, callback);
	});

	var beforeUpdate = function(params, callback) {
		var object;
		if (params.modifier) {
			params.modifier = utils.deepClone(params.modifier);
			...
			params.replacement = utils.deepClone(params.replacement);
