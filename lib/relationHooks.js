...
var setupRelationHooks = function(relatedCollection, relation) {
	...
	var shouldUpdate = function(modifier) {
		...
		if (utils.isModifier(modifier)) {
			modifiedFields = Object.keys(modifier).reduce(function(fields, key) {
				return fields.concat(
					getTopLevelFields(Object.keys(modifier[key]))
				);
			}, []);
		} else {
			modifiedFields = getTopLevelFields(Object.keys(modifier));
		}

		return projectionKeys.some(function(key) {
			return modifiedFields.indexOf(key) >= 0;
		});
	};

	var beforeUpdate = function(params, callback) {
		params.meta.modifiedIdentifiers = [];

		if (relation.onUpdate === 'cascade' && shouldUpdate(params.modifier)) {
