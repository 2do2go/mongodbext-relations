...
var prepareRelations = function(relations) {
	...
	Object.keys(relations).forEach(function(field) {
		...
		if (fieldParts.length > 1) {
			var itemPart = fieldParts[1] ? fieldParts[1] + '.' : '.';
			itemPart = 'item' + itemPart;

			preparedRelation.paths.modifier = field.replace('.$', '.$[item]');
			preparedRelation.paths.arrayFilter = itemPart + preparedRelation.key;
		}
